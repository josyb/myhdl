...
class TestBinaryOps(TestCase):
    ...
    def binaryBench(self, m, n):
        ...
        binops = binaryOps(
                           ...
                           Or,
                           left,
                           right).convert(hdl='SystemVerilog')
...
class TestMultiOps(TestCase):
    ...
    def multiBench(self, m, n, p):
        ...
        multiops = multiOps(
                           ...
                           argm,
                           argn,
                           argp).convert(hdl='SystemVerilog')
...
def augmOps(
    ...
    @instance
    def comb():
        var1 = intbv(0)[max(64, len(left) + len(right)):]
        while 1:
            yield left, right
            var1[:] = left
            # NOTE:
...
class TestAugmOps(TestCase):
    ...
    def augmBench(self, m, n):
        ...
        augmops = augmOps(
                           ...
                           Sub,
                           Sum,
                           left, right).convert(hdl='SystemVerilog')
...
if __name__ == '__main__':
    ...
    # Sum = Signal(intbv(0)[max(m, n) + 1:])
    # Sum_v = Signal(intbv(0)[max(m, n) + 1:])
    # augmOps(Bitand, Bitor, Bitxor, FloorDiv, LeftShift, Mod, Mul, RightShift, Sub, Sum, left, right).convert(hdl='SystemVerilog')
